...
class ResponseTransportRouteEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.expected_size,bool) or not isinstance(self.expected_size,int) or self.expected_size<=0:
    ...
    def qualified(self) -> bool:
        ...
        if self.status is None or not (200<=self.status<400):
            return False
        # A missing Content-Length does not make the route scientifically unusable:
        # exact payload size and digest are already frozen independently and the later
        # GET remains bounded. But a contradictory explicit length fails closed.
        if (
