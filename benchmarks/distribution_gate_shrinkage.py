"""Synthetic fallback benchmark for the experimental EOG distribution model.
...
single environmental predictor.  The training-fitted structural gate should therefore
shrink to zero and EOG should reduce exactly to the environmental support model.

This is a method-safety contract, not evidence about any empirical species.
"""
...
def build_environment_only_landscape(n_modules: int = 6):
    ...
    for module in range(n_modules):
        ...
        rows = [
            (f"{prefix}_source_a", 0.00, 0.90, 1, True),
            (f"{prefix}_source_b", 0.05, 0.90, 1, True),
            (f"{prefix}_negative_a", 0.10, 0.10, 0, True),
            (f"{prefix}_negative_b", 0.15, 0.10, 0, True),
            (f"{prefix}_heldout_positive", 0.20, 0.90, 1, False),
            (f"{prefix}_heldout_negative", 0.25, 0.10, 0, False),
        ]
        for node_id, longitude, environment, label, training in rows:
    ...
    return nodes, observed_ids, observed_response, target_ids, target_response


def run_gate_shrinkage_benchmark(n_modules: int = 6) -> dict[str, object]:
    nodes, observed_ids, observed_response, target_ids, target_response = (
    ...
    model = fit_eog_distribution(
        ...
        reference_provenance="environment-only structural gate shrinkage benchmark",
    )
    ...
    return {
        "schema": "eog_distribution_gate_shrinkage_v0_1",
        "n_modules": n_modules,
        ...
        "claim_boundary": (
            "synthetic fallback contract only; demonstrates exact reduction to the "
            "environmental model when the structural field is uninformative"
        ),
    }
