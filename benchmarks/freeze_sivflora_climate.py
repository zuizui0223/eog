#!/usr/bin/env python3
"""Freeze CHELSA v2.1 and WorldClim v2.1 2.5m at 22 SIVFLORA nodes.

This stage is response-blind: it consumes only the frozen node table and public climate
rasters. Missing/nodata at any declared node/variable is a hard block; there is no
nearby-cell substitution, locality substitution, resolution change, or imputation.
"""
...
def _load_nodes(path: Path) -> tuple[list[dict[str, str]], list[tuple[float, float]]]:
    with path.open(newline="", encoding="utf-8") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != EXPECTED_NODE_COUNT:
        raise ValueError(f"expected {EXPECTED_NODE_COUNT} frozen SIVFLORA nodes, got {len(rows)}")
    required = {"island_id", "acronym", "node_name", "latitude", "longitude"}
    ...
    for row in rows:
        ...
        if not math.isfinite(lat) or not math.isfinite(lon):
            raise ValueError(f"non-finite node coordinate for {row['node_name']}")
        coords.append((lon, lat))
    return rows, coords


def _probe(url: str) -> dict[str, object] | None:
    headers = {"User-Agent": "eog-sivflora-climate-freeze/1.0"}
    for request in (
...
def _chelsa_url(variable: str) -> dict[str, object]:
...
def _sample_raster(path_or_url: str, coords: list[tuple[float, float]]) -> tuple[np.ndarray, dict[str, object]]:
    raster_env = {
...
def _download(url: str, path: Path) -> dict[str, object]:
    request = urllib.request.Request(url, headers={"User-Agent": "eog-sivflora-climate-freeze/1.0"})
...
def _worldclim_member(variable: str, names: list[str]) -> str:
...
def freeze_climate(nodes_csv: Path, output_csv: Path, output_manifest: Path) -> dict[str, object]:
    nodes, coords = _load_nodes(nodes_csv)
    values: dict[str, np.ndarray] = {}
    chelsa_sources: list[dict[str, object]] = []

    for variable in VARIABLES:
        source = _chelsa_url(variable)
        sampled, raster_meta = _sample_raster(str(source["url"]), coords)
        missing = np.where(~np.isfinite(sampled))[0].tolist()
        if missing:
            names = [nodes[i]["node_name"] for i in missing]
            raise RuntimeError(f"CHELSA {variable} missing/nodata at frozen nodes: {names}")
        values[f"chelsa_{variable}"] = sampled
        chelsa_sources.append({**source, **raster_meta, "variable": variable})

    with tempfile.TemporaryDirectory(prefix="sivflora_worldclim_") as temp_dir:
        temp = Path(temp_dir)
        archive = temp / "wc2.1_2.5m_bio.zip"
        archive_headers = _download(WORLDCLIM_ARCHIVE_URL, archive)
        archive_sha = _sha256(archive)
        selected_members: list[dict[str, object]] = []
        with zipfile.ZipFile(archive) as bundle:
            names = bundle.namelist()
            for variable in VARIABLES:
                member = _worldclim_member(variable, names)
                extracted = temp / Path(member).name
                with bundle.open(member) as source_handle, extracted.open("wb") as target:
                    shutil.copyfileobj(source_handle, target, length=1024 * 1024)
                member_sha = _sha256(extracted)
                sampled, raster_meta = _sample_raster(str(extracted), coords)
                missing = np.where(~np.isfinite(sampled))[0].tolist()
                if missing:
                    names_missing = [nodes[i]["node_name"] for i in missing]
                    raise RuntimeError(
                        f"WorldClim 2.5m {variable} missing/nodata at frozen nodes: {names_missing}"
                    )
    ...
    manifest = {
        "status": "preoutcome_climate_freeze",
        "nodes_csv_sha256": _sha256(nodes_csv),
        "n_nodes": len(nodes),
        "variables": list(VARIABLES),
        "sampling_rule": "direct sample at frozen SIVFLORA islands_data coordinate",
...
def main() -> None:
    ...
    print(json.dumps(freeze_climate(args.nodes, args.output_csv, args.output_manifest), indent=2, sort_keys=True))
