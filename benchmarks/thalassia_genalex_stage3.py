...
IMPORTANT: this module is designed to be committed and tested before the released
microsatellite workbook is opened.  The real workbook must only be supplied after the
Stage-3 contract commit is frozen.
"""
...
class GenAlExSchemaError(ValueError):
    """The workbook does not satisfy the frozen Stage-3 GenAlEx contract."""
...
def _parse_allele(value: object, *, label: str) -> int:
    allele = _strict_int(value, label=label, minimum=0)
    return allele
...
def _parse_candidate_sheet(
    ...
    *,
    ...
    # Candidate identification uses only the standard GenAlEx parameter cells.
    try:
    ...
    for row_index in range(data_start, data_end + 1):
        ...
        for locus_index, locus_name in enumerate(locus_order):
            ...
            if (a == MISSING_ALLELE) != (b == MISSING_ALLELE):
                raise GenAlExSchemaError(
                    f"partial missing diploid genotype is prohibited at row {row_index}, locus {locus_name}"
                )
            genotypes.append(None if a == MISSING_ALLELE else tuple(sorted((a, b))))
        samples.append(SampleRecord(sample_id, population_id, tuple(genotypes)))

    # GenAlEx raw population data must be contiguous and agree with row-1 sizes / row-2 labels.
    cursor = 0
    ...
    # Do not silently consume an undeclared second raw block from the same sheet.
...
def complete_case_clone_correct(
    ...
    *,
    ...
    """Drop incomplete samples, then retain one copy of each exact within-population MLG."""
    ...
    for population_id in by_population:
        ...
        retained.sort(key=lambda sample: sample.sample_id)
        if len(retained) < 2:
            raise GenAlExSchemaError(
    ...
    *,
    ...
    for result in pairwise:
        ...
        if not math.isfinite(theta) or theta >= 1.0:
            raise GenAlExSchemaError(
                f"non-estimable pairwise theta for {result.population_a}-{result.population_b}: {theta}"
            )
        linearized = theta / (1.0 - theta)
        if not math.isfinite(linearized):
            raise GenAlExSchemaError("non-finite linearized FST")
        rows.append(
            {
    ...
    summary: dict[str, object] = {
        ...
        **qc,
        "clone_rule": f"within-population exact complete {len(parsed.locus_order)}-locus MLG; retain lexicographically smallest sample ID",
        "missing_rule": "GenAlEx 0/0 is missing; complete-case samples only before clone correction; partial 0/x is invalid",
        "fst_estimator": "multilocus Weir-Cockerham (1984) theta; no clipping",
        "response_transform": "theta/(1-theta); negative theta retained; theta>=1 is non-estimable",
    }
