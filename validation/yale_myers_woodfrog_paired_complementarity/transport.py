"""Dryad transport boundary for the Yale--Myers wood-frog attempt.
...
which returns independent S3 URLs for the four files in the fixed version.  This
module uses that manifest to keep the response object physically separate:

* preflight may download only the three declared nonresponse objects;
...
USER_AGENT = "EOG-yale-myers-woodfrog-frozen-transport/1.0"
...
def fetch_file_manifest(contract: dict, audit: dict) -> dict[str, str]:
    ...
    if set(observed) != set(declared):
        raise RuntimeError(
            "Dryad tokenized file manifest did not exactly match the closed four-file version"
        )
...
def download_nonresponse_member(
    ...
    if spec["role"] == "response":
        raise RuntimeError("response object cannot be opened through the nonresponse path")
