...
USER_AGENT = "EOG-Norway-Willow-Warbler-Endpoint3-Gate0/1.0"


class LiveMetadataStop(RuntimeError):
    """Technical or identity STOP before archive/member payload access."""
...
def _read_record_metadata(
    ...
    *,
    ...
    if parsed.scheme != "https" or parsed.hostname != host:
        raise LiveMetadataStop("authorized Zenodo metadata URL/host drift")
    ...
    try:
        ...
        raise LiveMetadataStop(f"Zenodo metadata transport unavailable: {exc}") from exc
    ...
    with response_context as response:
        ...
        if status != 200:
            raise LiveMetadataStop(
                f"Zenodo metadata GET returned HTTP {status}; response body was not opened"
            )
        if final.scheme != "https" or final.hostname != host:
            raise LiveMetadataStop(f"Zenodo metadata request left frozen host: {final.hostname!r}")
        if headers.get("content-encoding", "identity").casefold() != "identity":
            raise LiveMetadataStop("Zenodo metadata response unexpectedly used content encoding")
        ...
        if content_type not in {"application/json", "application/ld+json"}:
            raise LiveMetadataStop(
                f"Zenodo metadata response has unexpected content type {content_type!r}; body was not opened"
            )
        body = response.read(max_bytes + 1)
        ledger["metadata_bytes_opened"] = len(body)

    if len(body) > max_bytes:
        raise LiveMetadataStop("Zenodo metadata JSON exceeds frozen byte ceiling")
    try:
        metadata = json.loads(body.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise LiveMetadataStop(f"Zenodo metadata is not valid UTF-8 JSON: {exc}") from exc
    if not isinstance(metadata, dict):
        raise LiveMetadataStop("Zenodo metadata root is not a JSON object")
...
def run(
    ...
    *,
    ...
    if authorization.get("authorized_url") != contract["source"]["record_api_url"]:
        raise RuntimeError("authorization URL does not match frozen source contract")
    expected_blob = authorization.get("source_contract_git_blob_sha")
    if not isinstance(expected_blob, str) or _git_blob_sha(contract_path) != expected_blob:
    ...
    try:
        metadata, ledger = _read_record_metadata(authorization, opener=opener)
    ...
    result["source_contract_git_blob_sha"] = expected_blob
    result["archive_payload_requests"] = 0
    result["archive_payload_bytes_opened"] = 0
    result["member_payload_requests"] = 0
    result["member_payload_bytes_opened"] = 0
