...
def _header_words(value: str) -> set[str]:
    # Split separators and camelCase before normalizing individual words.
...
def evaluate_station_crosswalk(
    ...
    for name in header:
        ...
        if normalized in forbidden_tokens or words & forbidden_tokens:
    ...
    identifier_columns = [
        ...
        for name in header
        if any(
            _normalized(name) == token
            or _normalized(name).startswith(token)
            or _normalized(name).endswith(token)
