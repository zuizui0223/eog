...
PROTOCOL_PATH = BASE / "neon_small_mammal_response_protocol_v2.json"
PROTOCOL_LOCK_PATH = BASE / "neon_small_mammal_response_protocol_lock_v2.json"
...
OUTPUT_PATH = BASE / "neon_small_mammal_response_result_v2.json"
...
def eligible_target_taxa(protocol: dict[str, Any]) -> tuple[set[str], dict[str, str], str]:
    ...
    expected_fp = str(
        protocol["target_taxa"]["require_source_taxonomy_fingerprint"]
    )
    ...
    exclusions = set(str(value) for value in protocol["target_taxa"]["exclusions"])
    ids: set[str] = set()
    names: dict[str, str] = {}
    ...
    expected_count = int(protocol["target_taxa"]["expected_target_taxon_count"])
...
def terminal_payload(
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "eog.world_survival_regime.neon_small_mammal.response_result.v2",
...
def main() -> int:
    ...
    try:
        ...
        if (
            forecast_lock["result_fingerprint"]
            != response_protocol["prerequisite_forecast_result_fingerprint"]
        ):
