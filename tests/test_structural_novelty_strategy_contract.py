...
def test_submission_checklist_records_completed_scientific_and_presentation_gates() -> None:
    ...
    assert re.search(r"- \[x\] Figure 1 revised to show", text)
    assert re.search(r"- \[x\] A-Islands result figure reports", text)
    assert re.search(r"- \[x\] Reference-tier panel reports R0/R1/R2/R3/C", text)
    assert "STRUCTURAL_VISUAL_QA_RECEIPT_2026-09-12.md" in text
    assert "Submission-day publisher visual QA" in text
