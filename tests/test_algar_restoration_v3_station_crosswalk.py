...
def test_country_header_is_not_mistaken_for_count_response():
    raw = (
        "Station,Country,Longitude,Latitude\n"
        + "\n".join(
            f"ALG{i:03d},CA,{-112-i/1000},{56+i/1000}"
            for i in range(1, 39)
        )
        + "\n"
    ).encode("utf-8")
    ...
    assert result["forbidden_biological_columns"] == []
    assert result["status"] == "station_crosswalk_ready_response_blind"
