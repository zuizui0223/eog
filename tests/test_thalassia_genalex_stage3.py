...
def test_zero_zero_missing_is_recognized_but_partial_missing_is_rejected(tmp_path):
    ...
    with pytest.raises(GenAlExSchemaError, match="partial missing"):
        _parse(broken)
...
def test_response_builder_retains_negative_theta_and_never_clips(monkeypatch, tmp_path):
    ...
    rows, summary = build_pairwise_response(parsed, population_order=("P1", "P2"))
    assert rows[0]["fst_theta"] == pytest.approx(-0.05)
    assert rows[0]["linearized_fst"] == pytest.approx(-0.05 / 1.05)
    assert summary["response_transform"].startswith("theta/(1-theta)")
