...
> **Glanville rejected exact world IDs as direct supervised features. Tvärminne Daphnia showed that label-invariant Layer B contains small non-redundant information beyond mean support but is substantially worse than a strong frozen RF. No fresh post-Daphnia system has yet completed the new paired `strong learner` versus `same learner + Layer B` endpoint.**
...
> **Categorical response-token handling is part of response semantics. Any strip/casefold/internal-whitespace normalization used by an empirical runner must be declared and fingerprinted before row-level outcome access. Unknown post-open tokens stop the attempt; they do not authorize parser repair and rerun.**
...
## Response-token schema is now a prospective Gate
...
The schema fingerprint is incorporated into the existing `response_semantics` freeze;
the 16-key `FrozenOutcomeAccessContract` surface is not expanded, preserving historical
...
3. freeze response semantics **including categorical token schema**;
4. establish process/source closure;
5. pass response-blind structural-scale/adequacy gates;
6. freeze Layer-A worlds and update policy;
7. freeze unchanged Layer B;
8. freeze strong learner, preprocessing and conventional features;
9. freeze same-learner + Layer-B augmentation;
10. freeze heldout outer units, count minima, metric and paired decision rule;
11. run response-free synthetic smoke and freeze exact runtime/runner identity;
12. pass the 16-key outcome-access authorization;
13. open response once;
14. apply the already-frozen token schema;
15. run exact count gate before any fit;
16. only if count gate passes, fit and score once.

Any undeclared schema token after step 13 stops the attempt before scientific scoring.
...
Do not rerun coyote, Daphnia or earlier stopped systems as independent confirmation.

After the generic response-token schema contract is merged, the next milestone is:

> **select a genuinely fresh system, freeze its categorical response token semantics before access, and complete the paired strong-learner complementarity endpoint once without redesign.**

## Stop rules

1. Preserve favourable, adverse, blocked, null/no-confirmed and non-estimable evidence.
2. Do not rescue opened endpoints by parser, registry, scale or model redesign and relabel the rerun independent.
